...
async function tinyGet<T>(endpoint: string, params: Record<string, string>, tentativas = 3): Promise<T> {
  if (!isConfigured()) {
  ...
  for (let tentativa = 1; tentativa <= tentativas; tentativa++) {
    try {
      ...
      ultimoErro = error;
      if (tentativa < tentativas) await sleep(RATE_LIMIT_DELAY_MS * tentativa * 4);
    }
  }
